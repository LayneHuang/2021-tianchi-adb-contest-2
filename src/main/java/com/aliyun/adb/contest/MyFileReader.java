...
//    }
    ...
    public MyFileReader nextMyFileReader;
    ...
    private static void test(Path path, int threadCount, long bufferSize, int fileCount) {
        long t = System.currentTimeMillis();
        MyFileReader[] myFileReaders = new MyFileReader[threadCount];
        for (int i = 0; i < threadCount; i++) {
            myFileReaders[i] = new MyFileReader(path, i, threadCount, bufferSize, fileCount);
